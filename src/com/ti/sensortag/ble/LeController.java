...
public enum LeController implements PropertyChangeListener {
	...
	private static ScanType getScanType() {
		// Some models provide only one scanResult per scan (Nexus 4, Nexus 7
		// 2013)
	...
	private void changeNotificationStatus(final BluetoothGattService gattService,
			final Sensor sensor, final boolean enable) {
		writeQueue.queueRunnable(new Runnable() {
			@Override
			public void run() {
				BluetoothGattCharacteristic dataCharacteristic = gattService
						.getCharacteristic(sensor.getData());
				logEvent(bluetoothGatt.setCharacteristicNotification(dataCharacteristic, true),
						"The notification status was changed.",
				...
				byte[] configValue = enable ? ENABLE_NOTIFICATION_VALUE
	...
	private void discoverServices(BluetoothDevice device) {
		...
		if (serviceDiscoveryRunning) {
			...
			activity.runOnUiThread(new Runnable() {
				public void run() {
					Toast.makeText(activity, "Unable to start service discovery.",
	...
	public synchronized void propertyChange(PropertyChangeEvent event) {
		if (event.getPropertyName().equals(Devices.NEW_DEVICE_ + CONNECTED.name())) {
			...
			if (numServices > 0) {
				Log.i(TAG, "Skipping service discovery since we already have " + numServices
						+ " services cached.");
	...
	private void changeSensorStatus(final BluetoothGattService service, final Sensor sensor,
