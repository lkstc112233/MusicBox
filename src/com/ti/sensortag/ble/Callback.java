 ...
 * 
 * If an exception is thrown from one of your callback methods, it will be
 * caught with catch-all exception handling in BluetoothGatt and logged with
 * priority warning.
 * 
 * This is unfortunate because you do not get a stack trace, and because log
 * messages at priority warning are easily overlooked. To compensate for this we
...
class Callback extends BluetoothGattCallback {
	...
	public void onCharacteristicRead(BluetoothGatt gatt,
		...
		try {
			this.leController.logEvent(status == BluetoothGatt.GATT_SUCCESS,
					"Successfully read characteristic.", "Failed at reading characteristic");

			// Barometer calibration values are read.
			List<Integer> cal = new ArrayList<Integer>();
			for (int offset = 0; offset < 8; offset += 2) {
				Integer lowerByte = characteristic.getIntValue(FORMAT_UINT8, offset);
				Integer upperByte = characteristic.getIntValue(FORMAT_UINT8, offset + 1); // Note:
																							// interpret
																							// MSB
																							// as
																							// signed.
			...
			for (int offset = 8; offset < 16; offset += 2) {
				Integer lowerByte = characteristic.getIntValue(FORMAT_UINT8, offset);
				Integer upperByte = characteristic.getIntValue(FORMAT_SINT8, offset + 1); // Note:
																							// interpret
																							// MSB
																							// as
																							// signed.
	...
	public void onConnectionStateChange(final BluetoothGatt gatt, final int status,
			final int newState) {
		try {
			String msg = String.format("Callback.onConnectionStateChange(%s, %s, %s);", gatt
					.getDevice().getAddress(), BluetoothGattUtils.decodeReturnCode(status),
	...
	public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
		try {
			this.leController.logEvent(
					status == GATT_SUCCESS,
					"Descriptor write gave status return code: "
	...
	public void onCharacteristicWrite(BluetoothGatt gatt,
		...
		try {
			this.leController.logEvent(
					status == GATT_SUCCESS,
					"Characteristic write gave status return code: "
							+ BluetoothGattUtils.decodeReturnCode(status));
	...
	/**
	 * NB: On the first service discovery between a server-client pair the
	 * discovery might take several seconds. It has been observed to take 8
	 * seconds. But on subsequent discovery calls, the client will have the
	 * services cached and it will take milliseconds.
	 * */
	@Override
